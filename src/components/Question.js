...
const Question = () => {
    const [mcq, setMcq] = useState(questions[i])
    // const [activeClass,setActiveClass]=useState(false)
    answers = mcq.incorrect_answers;
    answers.push(mcq.correct_answer)
    console.log(answers)
 
    return (
        <Fragment>
            <h2>Question {i + 1} of 20</h2>
            <p>Category : {mcq.category.replace(/[%.*+?^${}()203A|[\]\\]/g, " ")}</p>

            <Box component="fieldset" mb={3} borderColor="transparent">
                <Rating
                    name="simple-controlled"
                    value={difficultyLevel(mcq.difficulty)}
                />
            </Box>
            <h3>{decodeURI(mcq.question).replace("80s", " ").replace("%3F", "?")}</h3>
            <div>{(answers.map((a, i) => (
                <button key={i} value={a} onClick={e=>{
                    console.log(e.target.value)
                    if(e.target.value===mcq.correct_answer){
                        correct=true;
                    }
                    else{
                        incorrect=true;
            ...
            {!isCompleted ? <button onClick={() => {
                ...
                setMcq(questions[i])
            }}>Next</button> : <p>Congratulations Your Test is Completed!</p>}
